# Claude Code 任务：修复游戏流程问题 v4（Demo自动重置）
...
在 `game_loop_v3.py` 的 `__init__()` 或游戏启动时调用重置方法：
...
def reset_game_state(self):
    ...
    for char_id, state in character_states.items():
        state["stress"] = state.get("base_stress", 0)
        state["madness"] = state.get("base_madness", 0)
        state["location"] = "牢房区"
        state["alive"] = True
...
def start_game(self):
    ...
    # 生成三天大纲
    print("\n[系统] 正在生成三天大纲...")
    self.story_planner.generate_chapter_outline()
    print("[系统] 大纲生成完成!")
...
### 7.1 `world_state/current_day.json` 初始模板
...
### STATUS.md 更新（在"更新日志"部分最顶部添加）

在 `## 更新日志` 下方、现有 `### 2025-12-22 - 橘雪莉角色设定修正` 之前插入：

```markdown
### 2025-12-22 - 游戏流程问题修复 v4（Demo自动重置）

**修改文件**: `game_loop_v3.py`, `api/fixed_event_manager.py`, `events/fixed_events.yaml`
...
5. 残留数据干扰：每次测试需手动重置状态文件

**修复内容**:

1. **game_loop_v3.py**:
   - 新增 `reset_game_state()`: Demo 启动时自动重置所有状态
   - 新增 `scatter_npcs()`: 将 NPC 随机分散到各地点
   - 修改 `advance_time()`: 修复时段顺序 dawn→morning→noon→afternoon→evening→night
   - 修改 `_run_fixed_event()`: 支持 `trigger_npc_scatter` 和 `next_period`

2. **fixed_event_manager.py**:
   - `handle_event_transitions()` 添加 `next_period` 和 `trigger_npc_scatter` 支持
   - `_check_trigger()` 修复 `after_event` 类型判断（跳过 period 检查）

3. **fixed_events.yaml**:
   - `day1_awakening` 添加 `next_event: day1_morning_assembly`
   - `day1_intro` 添加 `trigger_npc_scatter: true`
   - 完善 `day1_dinner` 和 `day1_night` 事件定义
...
- ✅ dawn → morning 正常过渡（不跳过）
- ✅ after_event 链式触发正常
- ✅ NPC 分散到各地点（玩家能在其他地点遇到角色）
- ✅ day1_dinner 和 day1_night 正常触发
...
**v3 遗留问题修复**:
1. ✅ 每次启动 Demo 自动重置状态（无需手动清理）
2. ✅ dawn → morning 正常过渡（不再跳到 noon）
3. ✅ after_event 链式触发正常
4. ✅ NPC 分散到各地点（玩家能在其他地点遇到角色）
5. ✅ day1_dinner (event_count≥6) 正常触发
6. ✅ day1_night (period=night) 正常触发并进入第2天
...
| 文件 | 操作 |
|------|------|
| `game_loop_v3.py` | 修改 `advance_time()`，新增 `scatter_npcs()`，新增 `reset_game_state()` |
| `api/fixed_event_manager.py` | 修改 `handle_event_transitions()`，`_check_trigger()` |
| `events/fixed_events.yaml` | 修改事件定义，添加 `next_event`/`next_period`/`trigger_npc_scatter` |
