...
**变更**: Claude 3.5 Sonnet 已被 Anthropic 下线，升级到 Claude Sonnet 4

| 项目 | 旧值 | 新值 |
|------|------|------|
| 模型 | `claude-3-5-sonnet-20241022` | `claude-sonnet-4-20250514` |

**修改文件**:
- `config.py` - 更新 MODEL 配置

**原因**: 旧模型返回 404 错误（模型不存在）

---

### 2025-12-26 - 故事连续性系统 v11

**功能**: 解决故事碎片化问题，实现场景间的记忆延续
...
**修改文件**:
- 修改 `api/director_planner.py` - 增加叙事上下文加载和保存逻辑
- 修改 `prompts/director_planner_prompt.txt` - 添加结尾指导和连续性要求
- 修改 `game_loop_v3.py` - 显示场景结尾提示和延续要素
- 新增 `world_state/narrative_context.json` - 叙事上下文数据文件

**核心功能**:

1. **ScenePlan 新增字段**:
   - `ending_type`: "closed" | "open" | "cliffhanger"
   - `next_scene_hint`: 下一场景暗示文本
   - `carryover_elements`: 延续要素列表

2. **叙事上下文系统 (narrative_context.json)**:
   ```json
   {
     "last_scene": {...},          // 上一场景摘要
     "recent_scenes": [],           // 最近5个场景
     "character_recent_interactions": {},  // 角色最近互动
     "unresolved_topics": [],       // 未解决话题
     "active_atmosphere": "neutral"
   }
   ```

3. **场景记忆传递**:
   - `_load_narrative_context()`: 加载叙事上下文
   - `_build_narrative_memory_prompt()`: 构建记忆prompt传给AI
   - `_save_scene_to_narrative()`: 保存场景到叙事上下文

4. **结尾类型指导**:
   - **闭合式 (closed)**: 对话自然收尾，无悬念（仅限纯日常）
   - **开放式 (open)**: 对话可被打断，暗示后续发展（推荐80%场景使用）
   - **悬念式 (cliffhanger)**: 突发事件打断，强烈疑问（关键场景）
